...
from slowfast_modules.common import drop_path
from slowfast_modules.nonlocal_helper import Nonlocal, Local
            ...
            for i in range(self.num_blocks[pathway]):
                ...
                if hasattr(self, "pathway{}_nonlocal{}".format(pathway, i)):
                    ...
                    if self.nonlocal_group[pathway] > 1:
                        ...
                        x = x.permute(0, 2, 1, 3, 4)
            if hasattr(self, "pathway{}_local".format(pathway)):
                local = getattr(self, "pathway{}_local".format(pathway))
                x = local(x)
            output.append(x)
...
def test():
    ...
    y = net([torch.randn(2, 80, 52, 56, 56), torch.randn(2, 8, 208, 56, 56)])
    print(y[0].shape)
    print(y[1].shape)

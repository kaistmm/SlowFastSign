...
class Local(nn.Module):
    ...
    def forward(self, x):
        ...
        theta = self.conv_theta(x)
        pad_len = self.temp_win // 2
        pad = torch.zeros_like(theta[:, :, :pad_len, :, :])

        phi = self.conv_phi(x)
        phi = torch.cat([pad, phi, pad], dim=2)
        phi = torch.stack([torch.cat([phi[:,:,i-pad_len:i], phi[:,:,i+1:i+pad_len+1]], dim=2) for i in range(pad_len, pad_len + T)], dim=2)
        g = self.conv_g(x)
        g = torch.cat([pad, g, pad], dim=2)
        g = torch.stack([torch.cat([g[:,:,i-pad_len:i], g[:,:,i+1:i+pad_len+1]], dim=2) for i in range(pad_len, pad_len + T)], dim=2)

        theta = theta.view(N, self.dim_inner, T, -1)
        phi = phi.view(N, self.dim_inner, T, -1)
        g = g.view(N, self.dim_inner, T, -1)

        # (N, C, T, HxW) * (N, C, T, window*HxW) => (N, T, HxW, windowxHxW).
        theta_phi = torch.einsum("ncts,nctp->ntsp", (theta, phi))
        # For original Non-local paper, there are two main ways to normalize
        # the affinity tensor:
        #   1) Softmax normalization (norm on exp).
        #   2) dot_product normalization.
        if self.instantiation == "softmax":
            # Normalizing the affinity tensor theta_phi before softmax.
            theta_phi = theta_phi * (self.dim_inner**-0.5)
            theta_phi = nn.functional.softmax(theta_phi, dim=3)
        elif self.instantiation == "dot_product":
            spatial_temporal_dim = theta_phi.shape[3]
            theta_phi = theta_phi / spatial_temporal_dim
        else:
            raise NotImplementedError(
                "Unknown norm type {}".format(self.instantiation)
        ...
        # (N, T, HxW, windowxHxW) * (N, C, T, windowxHxW) => (N, C, T, HxW).
        theta_phi_g = torch.einsum("ntsg,nctg->ncts", (theta_phi, g))
